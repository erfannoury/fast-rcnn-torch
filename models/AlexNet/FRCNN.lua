...
local function create_model(opt)
    ...
    local conv1 = nn.SpatialConvolution(3, 96, 11, 11, 4, 4, 5, 5, 1)
    conv1.name = "conv1"
    ...
    local conv2 = nn.SpatialConvolution(96, 256, 5, 5, 1, 1, 2, 2, 2)
    conv2.name = "conv2"
    shared:add(conv2)
    shared:add(nn.ReLU(true))
    shared:add(nn.SpatialMaxPooling(3, 3, 2, 2, 1, 1):ceil())
    ...
    local conv3 = nn.SpatialConvolution(256, 384, 3, 3, 1, 1, 1, 1, 1)
    conv3.name = "conv3"
    shared:add(conv3)
    shared:add(nn.ReLU(true))

    local conv4 = nn.SpatialConvolution(384, 384, 3, 3, 1, 1, 1, 1, 2)
    conv4.name = "conv4"
    shared:add(conv4)
    shared:add(nn.ReLU(true))

    local conv5 = nn.SpatialConvolution(384, 256, 3, 3, 1, 1, 1, 1, 2)
    conv5.name = "conv5"
    shared:add(conv5)
    shared:add(nn.ReLU(true))
