...
local function create_model(opt)
    ...
    shared:add(conv1)
    if opt.use_bn then
        local bn1 = cudnn.SpatialBatchNormalization(96)
        bn1.name = "batchnorm1"
        shared:add(bn1)
    end
    ...
    shared:add(conv3)
    if opt.use_bn then
        local bn3 = cudnn.SpatialBatchNormalization(384)
        bn3.name = "batchnorm3"
        shared:add(bn3)
    end
    shared:add(cudnn.ReLU(true))
    ...
    shared:add(conv4)
    if opt.use_bn then
        local bn4 = cudnn.SpatialBatchNormalization(256)
        bn4.name = "batchnorm4"
        shared:add(bn4)
    end
    shared:add(cudnn.ReLU(true))
    ...
    local conv5 = cudnn.SpatialConvolution(
        256, 256, 3, 3, 1, 1, 1, 1, opt.groups)
    conv5.name = "conv5"
    shared:add(conv5)
    if opt.use_bn then
        local bn5 = cudnn.SpatialBatchNormalization(256)
        bn5.name = "batchnorm5"
        shared:add(bn5)
    end
    shared:add(cudnn.ReLU(true))
