...
function ParallelTrainer._trainBatch(inputs_cpu, labels_cpu, loss_weights_cpu, iter, lr)
    ...
    _loss_weights, loss_weights_cpu =
        utils:recursiveResizeAsCopyTyped(_loss_weights, loss_weights_cpu, "torch.CudaTensor")
    ...
    if iter % _iter_show == 0 then
        print(
            string.format(
                ...
                iter,
                _avg_cls_loss / _iter_show,
                _avg_reg_loss / _iter_show,
                training_time
            )
        )
        _avg_cls_loss = 0
