...
function SequentialTrainer:train()
    ...
    -- Loading the optim state if we are resuming training
    if config.resume_training then
        print("Loading the optmizer state for resuming training...")
        local optim_path =
            paths.concat(config.save_path, paths.basename(config.pre_trained_file, ".t7") .. "_optimState.t7")
...
function SequentialTrainer:_trainBatch(inputs_cpu, labels_cpu, loss_weights_cpu, iter, lr)
    ...
    self._loss_weights, loss_weights_cpu =
        utils:recursiveResizeAsCopyTyped(self._loss_weights, loss_weights_cpu, "torch.CudaTensor")
    ...
    if iter % _iter_show == 0 then
        print(
            string.format(
                "Iteration = %d, Classification Loss = %2.2f, Regression Loss = %2.2f, Time = %2.2f",
                iter,
                self._avg_cls_loss / _iter_show,
                self._avg_reg_loss / _iter_show,
                training_time
            )
        )
        self._avg_cls_loss = 0
