...
function Net:_makeParallel()
    ...
    local parallel_model =
        nn.DataParallelTable(1, true, true):cuda():add(self.model, gpus):threads(
        function()
...
function Net:_sanitize()
    ...
    for _, val in ipairs(list) do
        for name, field in pairs(val) do
            ...
            if name == "homeGradBuffers" then
                val[name] = nil
            end
            if name == "input_gpu" then
                val["input_gpu"] = {}
            end
            if name == "gradOutput_gpu" then
                val["gradOutput_gpu"] = {}
            end
            if name == "gradInput_gpu" then
                val["gradInput_gpu"] = {}
            end
