...
function NetworkWrapper:trainNetwork(db)
    ...
    if config.nGPU > 1 then
        local parallelTrainer =
            detection.ParallelTrainer(
                nn.CrossEntropyCriterion():cuda(),
                detection.WeightedSmoothL1Criterion():cuda(),
                roi
        )

        parallelTrainer:train()
    else
        local sequentialTrainer =
            detection.SequentialTrainer(
                nn.CrossEntropyCriterion():cuda(),
                detection.WeightedSmoothL1Criterion():cuda(),
                roi
        )
...
function NetworkWrapper:testNetwork(db)
    ...
    for i = 1, n_class do
        all_detections[i] = tds.Hash()
    end

    -- Adaptive class thresholds
    local thresholds = {}
    for i = 1, n_class do
        thresholds[i] = -math.huge
    end

    -- Min heaps for maintaining max_per_set constraint
    local heaps = {}
    for i = 1, n_class do
        heaps[i] = detection.heap(max_per_set)
    end
    ...
    for i = 1, n_image do
        ...
        print(
            string.format(
                "Image# = %d/%d, detection time = %2.3fs, misc time = %2.3fs",
                i,
                n_image,
                det_time,
                misc_time
    ...
    print(
        string.format(
            "%d images detected!, average detection time = %2.3fs, average misc time = %2.3fs",
            n_image,
            avg_det_time,
            avg_misc_time
        )
        ...
        for j = 1, n_image do
            if all_detections[i][j]:numel() ~= 0 then
                ...
                if sel_inds:numel() == 0 then
                    ...
                    all_detections[i][j] = all_detections[i][j]:index(
                        1, sel_inds)
                    -- apply nms
