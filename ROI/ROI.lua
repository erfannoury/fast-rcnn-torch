...
function ROI:create_roidb(db)
    ...
    for i = 1, #self._roidb do
        ...
        for c = 1, n_class do
            ...
            if c_inds:numel() > 0 then
                cur_targets:indexCopy(
                    1,
                    c_inds,
                    cur_targets:index(1, c_inds) -
                        means[c]:resize(1, means[c]:numel()):expand(torch.LongStorage {c_inds:numel(), means:size(2)})
                )
                cur_targets:indexCopy(
                    1,
                    c_inds,
                    cur_targets:index(1, c_inds):cdiv(
                        stds[c]:resize(1, stds[c]:numel()):expand(torch.LongStorage {c_inds:numel(), means:size(2)})
                    )
                )
            end
        end
