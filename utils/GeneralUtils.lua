...
function GeneralUtils:tableDeepCopy(tab)
    if type(tab) == "userdata" and tab.clone ~= nil then
        return tab:clone()
    end -- for dealing with tensors
    if type(tab) ~= "table" and type(tab) ~= "cdata" then
        return tab
    end
    local res
    if type(tab) == "table" then
        ...
        for k, v in pairs(tab) do
            res[self:tableDeepCopy(k)] = self:tableDeepCopy(v)
        end
    ...
    qt.connect(
        ...
        function(x, y)
            for i = 1, boxes_thresh:size(1) do
                if
                    x > boxes_thresh[i][1] and x < boxes_thresh[i][3] and y > boxes_thresh[i][2] and
                        y < boxes_thresh[i][4]
                 then
                    w:setcolor(200 / 255, 130 / 255, 200 / 255, 1)
                    w:rectangle(20, 20, 120, 55)
...
function GeneralUtils:recursiveResizeAsCopyTyped(t1, t2, type)
    ...
    elseif torch.isTensor(t2) then
        ...
        error("expecting nested tensors or tables. Got " .. torch.type(t1) .. " and " .. torch.type(t2) .. " instead")
    end
    return t1, t2
end
...
function GeneralUtils:boxoverlap(a, b)
    ...
    local aarea = torch.cmul((a:select(2, 3) - a:select(2, 1) + 1), (a:select(2, 4) - a:select(2, 2) + 1)):float()
...
function GeneralUtils:nms(boxes, overlap)
    ...
    while I:numel() > 0 do
        ...
        w:map2(
            xx2,
            xx1,
            function(xx, xxx2, xxx1)
                return math.max(xxx2 - xxx1 + 1, 0)
            end
        )
        h:resizeAs(yy2):zero()
        h:map2(
            yy2,
            yy1,
            function(xx, yyy2, yyy1)
                return math.max(yyy2 - yyy1 + 1, 0)
            end
        )
